...
echo "  INFO  codex: --profile flag (verified from source/docs)"
echo "  INFO  gemini: auto-fallback chain (verified from source/docs)"

echo ""
echo "--- Subagent Model Override ---"
echo "  INFO  claude: CLAUDE_CODE_SUBAGENT_MODEL env var (verified via official docs)"
echo "  INFO  cursor: subagent model config (verified via cursor.com/docs/subagents)"

echo ""
echo "--- Deferred Tool Loading ---"
echo "  INFO  claude: MCP tool schemas deferred by default (verified via official docs)"
echo "  INFO  cursor: Dynamic Context Discovery (verified via cursor.com/blog/dynamic-context-discovery)"
echo "  INFO  gemini: skills progressive disclosure (verified via source/docs)"

echo ""
echo "--- Cache TTL ---"
echo "  INFO  claude: ENABLE_PROMPT_CACHING_1H env var (verified at code.claude.com/docs/en/env-vars)"
echo "  INFO  codex: automatic prompt caching via Responses API (verified from docs)"
echo "  INFO  gemini: automatic caching on API key + Vertex AI (verified from docs)"
...
echo ""
echo "--- Tool Output Limits ---"
echo "  INFO  codex: tool_output_token_limit in config.toml (verified from source/docs)"
echo "  INFO  gemini: distillation.maxOutputTokens in settings.json (verified from source/docs)"
echo "  INFO  gemini: summarizationThresholdTokens in settings.json (verified from source/docs)"

echo ""
echo "--- Codex Config Settings ---"
echo "  INFO  codex: model_auto_compact_token_limit in config.toml (verified from source/docs)"
echo "  INFO  codex: model_context_window in config.toml (verified from source/docs)"
echo "  INFO  codex: .codexignore file (verified from source/docs)"

echo ""
echo "--- Gemini Config Settings ---"
echo "  INFO  gemini: compressionThreshold in settings.json (verified from source/docs)"
echo "  INFO  gemini: thinkingConfig.thinkingBudget in settings.json (verified from source/docs)"
echo "  INFO  gemini: historyWindow.maxTokens in settings.json (verified from source/docs)"
