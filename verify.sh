...
echo ""
echo "--- Prompt Cache ---"
# CC: 10% of input, 5-min default TTL, 1-hour for subscribers — verified from Anthropic pricing docs
echo "  INFO  claude: prompt cache 10% / 5-min default TTL, 1-hour for subscribers (verified from Anthropic pricing docs)"
echo "  INFO  claude: ENABLE_PROMPT_CACHING_1H env var for API users (verified at code.claude.com/docs/en/env-vars)"
echo "  INFO  codex: auto caching for prompts >= 1024 tokens, up to 90% discount (verified from OpenAI docs)"
echo "  INFO  gemini: 90% cached-input discount, 2x price above 200K for Pro (verified from Google Cloud pricing)"
...
echo "  INFO  codex: model_reasoning_effort and plan_mode_reasoning_effort in config.toml (verified from developers.openai.com/codex/config-reference)"
echo "  INFO  gemini: /think [off|low|med|high|max] and /budget <tokens> slash commands (verified from github.com/google-gemini/gemini-cli)"
...
echo ""
echo "--- Fast Mode ---"
echo "  INFO  claude: /fast toggle (verified at code.claude.com/docs/en/fast-mode)"
echo "  INFO  claude: CLAUDE_CODE_DISABLE_FAST_MODE env var (verified at code.claude.com/docs/en/env-vars)"
echo "  INFO  codex: /fast [on|off|status] (verified from developers.openai.com/codex/cli/slash-commands)"

echo ""
echo "--- New Slash Commands (June 2026) ---"
echo "  INFO  cursor: /btw (verified from cursor.com/changelog/04-14-26)"
echo "  INFO  cursor: /debug (verified from cursor.com/changelog/04-14-26)"
echo "  INFO  cursor: /worktree (verified from cursor.com/changelog/3-0)"
echo "  INFO  cursor: /best-of-n (verified from cursor.com/changelog/3-0)"
echo "  INFO  cursor: /config (verified from cursor.com/changelog/04-14-26)"
echo "  INFO  cursor: /statusline (verified from cursor.com/changelog/04-14-26)"
echo "  INFO  cursor: /auto-run (verified from cursor.com/docs/cli/reference/slash-commands)"
echo "  INFO  codex: /fork and /side (verified from developers.openai.com/codex/cli/slash-commands)"
echo "  INFO  codex: /goal (verified from developers.openai.com/codex/cli/slash-commands)"
echo "  INFO  codex: /memories (verified from developers.openai.com/codex/memories)"
echo "  INFO  codex: /archive (verified from developers.openai.com/codex/changelog)"
echo "  INFO  codex: hooks.json now 10 events (verified from developers.openai.com/codex/hooks)"
echo "  INFO  gemini: /think and /budget (verified from github.com/google-gemini/gemini-cli)"
echo "  INFO  gemini: /rewind (verified from github.com/google-gemini/gemini-cli/docs/reference/commands.md)"
echo "  INFO  gemini: /export-session and --session-file (verified from gemini-cli v0.43.0 release)"
echo "  INFO  gemini: /extensions system (verified from github.com/google-gemini/gemini-cli/docs/extensions)"
echo "  INFO  gemini: plan.modelRouting setting (verified from github.com/google-gemini/gemini-cli/docs/reference/configuration.md)"
echo "  INFO  gemini: /memory inbox and 4-tier memory (verified from gemini-cli v0.40.0)"
echo "  INFO  gemini: compressionThreshold default 0.5 (verified from gemini-cli v0.38.0)"
