...
echo ""
echo "--- Prompt Cache ---"
# CC: 10% of input, 1h TTL for subscription, 5-min for API key — verified from code.claude.com/docs/en/prompt-caching
echo "  INFO  claude: prompt cache 10% / 1h subscription TTL / 5m API key TTL (verified from code.claude.com/docs/en/prompt-caching)"
echo "  INFO  claude: promptCacheTtl / CLAUDE_CODE_PROMPT_CACHE_TTL settings (verified from code.claude.com/docs/en/prompt-caching#choose-the-ttl-yourself)"
...
echo "  INFO  claude: /effort is an internal slash command with low/med/high/xhigh/max/ultracode (verified via code.claude.com/docs/en/model-config)"
echo "  INFO  claude: maxEffortLevel managed setting for org-wide cap (verified via code.claude.com/docs/en/model-config)"
echo "  INFO  cursor: /max-mode is an interactive slash command (verified in agent CLI)"
echo "  INFO  codex: model_reasoning_effort config.toml key (verified from source codex-rs)"
echo "  INFO  gemini: thinkingBudget in generationConfig (verified from source docs/cli/generation-settings.md)"

echo ""
echo "--- Hooks ---"
echo "  INFO  claude: hooks in settings.json, 30+ events, 4 hook types (verified via code.claude.com/docs/en/hooks)"
echo "  INFO  claude: PreModelSwitch + PostToolBatch hook events (verified via code.claude.com/docs/en/hooks)"
echo "  INFO  cursor: .cursor/hooks.json (verified via cursor.com/docs/hooks)"
echo "  INFO  codex: .codex/hooks.json, 12 events, command type (verified from source codex-rs/hooks/src/lib.rs)"
echo "  INFO  gemini: hooks in settings.json, 11 events, 3 types: command/plugin/runtime (verified from source docs/hooks/reference.md)"
...
echo "--- Context Window, Thinking & Budget Controls ---"
...
echo "  INFO  claude: bashOutputMaxChars / taskOutputMaxChars settings (verified at code.claude.com/docs/en/costs)"
echo "  INFO  claude: maxEffortLevel managed setting (verified at code.claude.com/docs/en/model-config)"
echo "  INFO  codex: model_auto_compact_token_limit config key (verified from source codex-rs)"
echo "  INFO  codex: tool_output_token_limit config key (verified from source codex-rs)"
echo "  INFO  gemini: thinkingBudget max 8192 (verified from source packages/core/src/config/models.ts)"
echo "  INFO  gemini: compactToolOutput setting (verified from source settingsSchema.ts)"
...
echo ""
echo "--- Track Spend ---"
echo "  INFO  claude: /usage (was /cost) with cache hit ratio + miss diagnostics (verified via code.claude.com/docs/en/costs)"
echo "  INFO  claude: /insights generates HTML usage report (verified via code.claude.com/docs/en/costs)"
echo "  INFO  cursor: /usage is an interactive slash command (verified in agent CLI)"
echo "  INFO  codex: /status, /usage, and /statusline are internal slash commands (verified from source)"
echo "  INFO  gemini: /stats session|model|tools is an internal slash command (verified from source)"
...
echo "  INFO  claude: /recap keeps cache intact (verified via code.claude.com/docs/en/prompt-caching)"
echo "  INFO  claude: /rewind truncates to earlier turn (verified via code.claude.com/docs/en/prompt-caching)"
echo "  INFO  claude: /skill-doctor shows unused skills + context cost (verified via code.claude.com/docs/en/changelog)"
echo "  INFO  claude: /fast toggles fast mode (verified via code.claude.com/docs/en/fast-mode)"
echo "  INFO  codex: /clear is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  codex: /side (alias /btw) ephemeral fork (verified from source codex-rs/tui)"
echo "  INFO  codex: /recap is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  codex: /usage is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  codex: /agents (plural) is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  codex: /fork is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  codex: /goal is an internal slash command (verified from source codex-rs/tui)"
echo "  INFO  gemini: /rewind is an internal slash command (verified from source, v0.27.0)"
echo "  INFO  gemini: /agents config per-agent control (verified from source docs/reference/commands.md)"
echo "  INFO  gemini: /hooks manage lifecycle hooks (verified from source docs/hooks/reference.md)"
