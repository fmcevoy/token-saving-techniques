...
echo "--- Context Window & Caching Env Vars ---"
...
echo "  INFO  codex: /fast and /slow are internal slash commands (verified from source)"

echo ""
echo "--- Output Verbosity ---"
echo "  INFO  codex: model_verbosity in config.toml (verified from source)"
echo "  INFO  gemini: summarizeToolOutput in settings.json (verified from source)"

echo ""
echo "--- Persist Decisions ---"
echo "  INFO  claude: /memory is an internal slash command (verified via official docs)"
echo "  INFO  gemini: /memory show and /memory refresh are internal slash commands (verified from source)"

echo ""
echo "--- Track Spend ---"
echo "  INFO  claude: /usage (merged /cost + /stats) is an internal slash command (verified via official docs)"
