...
echo ""
echo "--- Prompt Cache ---"
# CC: 10% of input, 1h TTL (subscription) / 5-min TTL (API) — verified from Anthropic pricing docs
echo "  INFO  claude: prompt cache 10% / 1h TTL (subscription) / 5m TTL (API) (verified from Anthropic pricing docs)"
echo "  INFO  claude: ENABLE_PROMPT_CACHING_1H=1 env var (verified at code.claude.com/docs/en/costs)"
...
echo "  INFO  claude: CLAUDE_AUTOCOMPACT_PCT_OVERRIDE (verified at code.claude.com/docs/en/costs)"
...
echo "  INFO  claude: CLAUDE_CODE_GOAL_CHECKIN_MINUTES (verified at code.claude.com/docs/en/costs)"
...
echo "  INFO  gemini: /memory is an internal slash command with list/refresh/show subcommands (verified from source)"

echo ""
echo "--- Track Spend ---"
echo "  INFO  claude: /usage and /cost are internal slash commands (verified via official docs)"
...
echo "  INFO  gemini: /stats is an internal slash command with session/model/tools subcommands (verified from source)"
