...
echo "  INFO  claude: prompt cache 10% / 1-hour TTL subscribers, 5-min API (verified at code.claude.com/docs/en/prompt-caching)"
...
echo "  INFO  claude: /undo is an alias for /rewind (verified via official docs)"
...
echo "  INFO  claude: /usage is an internal slash command (verified via official docs — Week 16/21 digests)"
