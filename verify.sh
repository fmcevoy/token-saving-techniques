...
echo ""
echo "--- Rewind ---"
echo "  INFO  claude: /rewind is an internal slash command (verified via official docs — aliases: /checkpoint, /undo)"
echo "  INFO  gemini: /rewind is an internal slash command (verified from source — also Esc Esc)"
echo "  INFO  codex: /fork is an internal slash command (verified from official docs)"

echo ""
echo "--- Prompt Cache ---"
# CC: 10% of input, 5-min default TTL, 1-hour opt-in — verified from Anthropic pricing docs
echo "  INFO  claude: prompt cache 10% read / 1.25x 5-min write / 2x 1-hour write (verified from Anthropic pricing docs)"
echo "  INFO  claude: ENABLE_PROMPT_CACHING_1H env var (verified at code.claude.com/docs/en/env-vars)"
...
echo "  INFO  gemini: auto mode routes Flash/Pro by complexity (verified from source)"
...
check claude "CC --effort xhigh" "command claude --help" "xhigh"
echo "  INFO  claude: /effort is an internal slash command with xhigh level (verified via official docs)"
echo "  INFO  claude: CLAUDE_CODE_EFFORT_LEVEL env var (verified at code.claude.com/docs/en/env-vars)"
echo "  INFO  claude: ultrathink keyword triggers deep reasoning (verified via official docs)"
...
echo ""
echo "--- Fast Mode ---"
echo "  INFO  claude: /fast toggles fast mode (6x cost, 2.5x speed — verified via official docs)"
...
echo "  INFO  claude: CLAUDE_CODE_MAX_OUTPUT_TOKENS env var (verified at code.claude.com/docs/en/env-vars)"
echo "  INFO  claude: CLAUDE_CODE_MAX_TURNS env var (verified at code.claude.com/docs/en/env-vars)"

echo ""
echo "--- Restrict Tools ---"
check claude "CC --tools flag" "command claude --help" "--tools"
...
echo "  INFO  gemini: model.compressionThreshold via /settings (verified from source — default 0.5)"
...
echo ""
echo "--- Track Spend ---"
echo "  INFO  claude: /usage (/cost, /stats are aliases) is an internal slash command (verified via official docs)"
...
echo "  INFO  claude: /batch is a bundled skill (verified via official docs)"
echo "  INFO  claude: /background is an internal slash command (verified via official docs)"
echo "  INFO  cursor: /btw is an interactive slash command (verified in Cursor 3.1 changelog — Apr 14, 2026)"
echo "  INFO  cursor: /multitask is an interactive slash command (verified in Cursor 3.2 changelog — Apr 24, 2026)"
echo "  INFO  cursor: /worktree is an interactive slash command (verified in Cursor 3.0 changelog — Apr 2, 2026)"
echo "  INFO  cursor: /config is an interactive slash command (verified in Cursor 3.1 changelog)"
echo "  INFO  cursor: /mcp is an interactive slash command (verified in agent CLI)"
echo "  INFO  codex: /goal is an experimental slash command (verified from official docs — needs features.goals)"
echo "  INFO  codex: /fork is an internal slash command (verified from official docs)"
echo "  INFO  codex: /side is an internal slash command (verified from official docs)"
echo "  INFO  codex: /memories is an internal slash command (verified from official docs)"
echo "  INFO  codex: /fast is an internal slash command (verified from official docs)"
echo "  INFO  gemini: /rewind is an internal slash command (verified from source)"
echo "  INFO  gemini: /checkpoint is an internal slash command (verified from source)"
...
echo "  INFO  gemini: /settings is an internal slash command (verified from source)"
