...
echo "  INFO  cursor: /model auto with Cost/Balance/Intel modes (verified via cursor.com/changelog/router)"
echo "  INFO  codex: model tiers in config.toml (verified from source)"
echo "  INFO  gemini: plan mode auto-routes Pro/Flash (verified from source)"

echo ""
echo "--- Token & Agent Budget Caps ---"
echo "  INFO  claude: CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION (verified at code.claude.com/docs/en/changelog)"
echo "  INFO  claude: CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS (verified at code.claude.com/docs/en/changelog)"
echo "  INFO  codex: [features.rollout_budget] in config.toml (verified from GitHub PRs)"
echo "  INFO  gemini: model.maxSessionTurns in settings.json (verified from source)"
echo "  INFO  gemini: billing.overageStrategy in settings.json (verified from source)"
...
echo "  INFO  claude: /config is an internal slash command (verified via official docs)"
