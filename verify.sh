...
check codex "Codex --reasoning-effort flag" "codex exec --help" "reasoning"
...
echo "  INFO  gemini: thinkingLevel in settings.json modelConfigs (verified from source)"
...
echo "  INFO  claude: model: frontmatter in SKILL.md (verified at code.claude.com/docs/en/sub-agents)"
echo "  INFO  cursor: /multitask parallel subagents (verified at cursor.com/changelog)"
echo "  INFO  codex: /agent /fork /side slash commands (verified at developers.openai.com/codex/subagents)"
echo "  INFO  gemini: model: in agent frontmatter (verified at github.com/google-gemini/gemini-cli/docs/core/subagents.md)"

echo ""
echo "--- Fallback Models ---"
check claude "CC --fallback-model flag" "command claude --help" "fallback"
echo "  INFO  claude: fallbackModel setting (array of up to 3 models, verified at code.claude.com/docs/en/model-config)"

echo ""
echo "--- New Cost Features ---"
echo "  INFO  claude: /usage per-category breakdown (verified at code.claude.com/docs/en/changelog)"
echo "  INFO  codex: /goal objective tracking with budget (verified at developers.openai.com/codex/cli/features)"
echo "  INFO  codex: /archive session protection (verified at developers.openai.com/codex/cli/slash-commands)"
echo "  INFO  gemini: /checkpoint and /restore (verified at github.com/google-gemini/gemini-cli/docs/reference/commands.md)"
echo "  INFO  gemini: /stats session|model|tools subcommands (verified from source)"

echo ""
echo "--- Prompt Cache ---"
echo "  INFO  claude: ENABLE_PROMPT_CACHING_1H=1 for 1-hour TTL (verified at code.claude.com/docs/en/prompt-caching)"
echo "  INFO  gemini: Implicit caching on 2.5+ models, needs API key auth (verified at github.com/google-gemini/gemini-cli/docs/cli/token-caching.md)"
...
echo ""
echo "--- Track Spend ---"
echo "  INFO  claude: /cost and /usage are internal slash commands (verified via official docs)"
